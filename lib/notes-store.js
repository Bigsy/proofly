...
// Every full record in the library, index order (for "Export all"). Reads the
// index first so orphan bodies (invisible in the library) stay invisible here.
export async function listNotes() {
  const s = store();
  if (!s) return [];
  const index = await listIndex();
  if (!index.length) return [];
  const keys = index.map((e) => noteKey(e.id));
  let data;
  try {
    data = await s.get(keys);
  } catch {
    return [];
  }
  return index
    .map((e) => data?.[noteKey(e.id)])
    .filter((n) => n && typeof n === "object" && typeof n.id === "string");
}

// Upsert a batch of notes KEEPING their own timestamps (unlike saveNote, which
// stamps `now`) — the import path, where updatedAt is the note's provenance and
// decides the library order. Same write ordering as saveNote: every body lands
// before the single index rewrite, so a failure only ever orphans bodies.
// Callers decide which notes to write (see planImport); this writes all given.
export async function mergeNotes(notes) {
  const s = store();
  if (!s) return [];
  const records = [];
  const seen = new Set();
  ...
  const index = (await readIndex(s)).filter((e) => validEntry(e) && !seen.has(e.id));
