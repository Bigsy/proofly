...
export class GitHubContentClient {
  ...
  async request(path, options = {}) {
    ...
    const res = await this.fetch(`${API}${path}`, {
      ...options,
      // Contents API writes are optimistic: a 409/422 retry must re-read the
      // current blob SHA. Browser HTTP caches can otherwise replay the exact
      // stale GET which caused the conflict and make the retry fail forever.
